const getToken = () => localStorage.getItem('gestao_clin_token')
...
export async function apiFetch<T = any>(path: string, opts: RequestInit = {}): Promise<T> {
  ...
  if (res.status === 401) { localStorage.removeItem('gestao_clin_token'); window.location.href = `${BASE}/login`; throw new Error('Unauthorized') }
...
export interface Account { id: number; name: string; slug: string; logo_url: string | null; is_active: number; created_at: string; lead_count?: number; user_count?: number }
...
export const createAccount = (name: string) => apiFetch<{ account: Account }>('/api/accounts', { method: 'POST', body: JSON.stringify({ name }) }).then(d => d.account)
...
export const createBroadcast = (accountId: number, data: { name: string; message_template: string; lead_ids: number[] }) => apiFetch(`/api/broadcasts?account_id=${accountId}`, { method: 'POST', body: JSON.stringify(data) })
