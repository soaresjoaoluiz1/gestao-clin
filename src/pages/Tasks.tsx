...
const BUCKETS = [
  ...
  { key: 'later' as const, label: 'Mais tarde', color: '#A0AEC0', icon: Calendar },
]
    ...
    return (
      <div key={t.lead_cadence_id} className="card" style={{ padding: 14, marginBottom: 10 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          {/* Avatar */}
          <div style={{ width: 40, height: 40, borderRadius: '50%', background: t.stage_color ? `${t.stage_color}25` : '#5CB8B225', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
            {t.profile_pic_url ? (
              ...
              <User size={18} style={{ color: t.stage_color || '#5CB8B2' }} />
            )}
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 11, color: '#718096', display: 'flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
                <Clock size={11} /> {formatDueTime(t.due_datetime)}
              </span>
            </div>

            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6, flexWrap: 'wrap' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px', background: '#5CB8B215', borderRadius: 6, fontSize: 11, color: '#5CB8B2', fontWeight: 600 }}>
              ...
              {t.stage_name && <span style={{ fontSize: 10, color: t.stage_color || '#718096', background: `${t.stage_color || '#718096'}15`, padding: '2px 6px', borderRadius: 6 }}>{t.stage_name}</span>}
            </div>

            {t.attempt_description && <div style={{ fontSize: 12, color: '#4A5568', marginBottom: 4 }}>{t.attempt_description}</div>}
            {t.attempt_instructions && <div style={{ fontSize: 11, color: '#718096', marginBottom: 4, fontStyle: 'italic' }}>{t.attempt_instructions}</div>}
        ...
        <div className="modal-overlay" onClick={() => setConfirmModal(null)}>
          <div className="modal" style={{ maxWidth: 420 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, textAlign: 'center', padding: '8px 0' }}>
              ...
              <div style={{ fontSize: 13, color: '#718096' }}>Para <b style={{ color: '#2D3748' }}>{confirmModal.leadName}</b></div>

              {confirmModal.nextStep ? (
                <div style={{ width: '100%', padding: 12, background: 'rgba(92,184,178,0.08)', border: '1px solid rgba(92,184,178,0.25)', borderRadius: 8, marginTop: 4 }}>
                  <div style={{ fontSize: 11, color: '#5CB8B2', fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>Próxima etapa</div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>
                    {ACTION_LABELS[confirmModal.nextStep.action_type] || confirmModal.nextStep.action_type}
                    {confirmModal.nextStep.description && <span style={{ color: '#718096', fontWeight: 400 }}> · {confirmModal.nextStep.description}</span>}
                  </div>
