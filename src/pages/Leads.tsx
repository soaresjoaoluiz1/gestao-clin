  ...
  return (
    <div>
        ...
        <div className="page-header-actions">
          <button
            ...
            {!showArchived && archivedCount.withActivity > 0 && (
              <span style={{ marginLeft: 6, background: '#5CB8B2', color: '#000', fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 10 }}>{archivedCount.withActivity} nova{archivedCount.withActivity > 1 ? 's' : ''}</span>
            )}
          </button>
      ...
      <div className="filter-bar" style={{ marginTop: -8 }}>
        ...
        <span style={{ color: '#A0AEC0', fontSize: 12 }}>ate</span>
      ...
      {selected.size > 0 && user?.role !== 'atendente' && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '10px 14px', background: 'rgba(92,184,178,0.08)', borderRadius: 8, marginBottom: 12, border: '1px solid rgba(92,184,178,0.15)' }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: '#5CB8B2' }}>{selected.size} selecionados</span>
      ...
      {loading ? <div className="loading-container"><div className="spinner" /></div> : isMobile ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div key={l.id} className="lead-mobile-card" onClick={() => navigate(`/leads/${l.id}`)} style={{ position: 'relative' }}>
              <div className="lead-mobile-card-header">
                <div>
                  ...
                  {l.phone && <div style={{ fontSize: 12, color: '#718096', marginTop: 2 }}>{l.phone}</div>}
                </div>
                <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexShrink: 0 }}>
                  ...
                  <button
                    ...
                    {l.is_archived && l.has_new_after_archive ? <span style={{ position: 'absolute', top: 2, right: 2, width: 6, height: 6, borderRadius: '50%', background: '#5CB8B2' }} /> : null}
                  </button>
                </div>
              </div>
          ...
          {leads.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: '#A0AEC0' }}>Nenhum lead encontrado</div>}
          {total > 30 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, padding: 12 }}>
              <button className="btn btn-secondary btn-sm" disabled={page <= 1} onClick={() => setPage(p => p - 1)}>Anterior</button>
              <span style={{ fontSize: 12, color: '#718096', padding: '8px 12px' }}>{page}/{Math.ceil(total / 30)}</span>
        ...
        <div className="table-card">
          <table>
            <thead><tr>
              {user?.role !== 'atendente' && <th style={{ width: 32 }}><button style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#718096' }} onClick={toggleSelectAll}>{selected.size === leads.length && leads.length > 0 ? <CheckSquare size={14} /> : <Square size={14} />}</button></th>}
            ...
            <tbody>
              {leads.map(l => (
                <tr key={l.id} style={{ cursor: 'pointer' }}>
                  {user?.role !== 'atendente' && <td onClick={e => { e.stopPropagation(); toggleSelect(l.id) }}><span style={{ color: selected.has(l.id) ? '#5CB8B2' : '#A0AEC0', cursor: 'pointer' }}>{selected.has(l.id) ? <CheckSquare size={14} /> : <Square size={14} />}</span></td>}
              ...
              {leads.length === 0 && <tr><td colSpan={user?.role !== 'atendente' ? 9 : 8} style={{ textAlign: 'center', padding: 40, color: '#A0AEC0' }}>Nenhum lead encontrado</td></tr>}
            </tbody>
          </table>
          {total > 30 && (
            <div style={{ padding: 12, display: 'flex', justifyContent: 'center', gap: 8 }}>
              <button className="btn btn-secondary btn-sm" disabled={page <= 1} onClick={() => setPage(p => p - 1)}>Anterior</button>
              <span style={{ fontSize: 12, color: '#718096', padding: '6px 12px' }}>Pagina {page} de {Math.ceil(total / 30)}</span>
              <button className="btn btn-secondary btn-sm" disabled={page >= Math.ceil(total / 30)} onClick={() => setPage(p => p + 1)}>Proxima</button>
            </div>
          )}
        </div>
