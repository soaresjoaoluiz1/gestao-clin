            ...
            {filteredLeads.map(l => {
              ...
              return (
                <div key={l.id} className={`chat-contact-item ${active ? 'active' : ''}`} onClick={() => setSelectedLeadId(l.id)} style={{ position: 'relative' }}>
                  <div className="chat-contact-avatar" style={{ background: stage ? `${stage.color}25` : '#5CB8B225', overflow: 'hidden' }}>
                    {l.profile_pic_url ? (
                      ...
                      <User size={16} style={{ color: stage?.color || '#5CB8B2' }} />
                    )}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 10, color: '#A0AEC0' }}>{timeAgo(l.updated_at)}</span>
                    </div>
                  ...
                  <button
                    ...
                    style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(0,0,0,0.35)', border: 'none', color: '#4A5568', cursor: 'pointer', padding: 4, borderRadius: 4, display: 'none' }}>
                    <Archive size={11} />
                  </button>
                </div>
              )
            })}
            {filteredLeads.length === 0 && <div style={{ padding: 30, textAlign: 'center', color: '#A0AEC0', fontSize: 12 }}>Nenhum contato</div>}
          </div>
        </div>
        ...
        <div className="chat-conversation">
          {!lead ? (
            <div className="chat-empty">
              <MessageCircle size={48} style={{ color: '#A0AEC0', marginBottom: 12 }} />
              <h3 style={{ fontSize: 16, color: '#718096' }}>Selecione um contato</h3>
            </div>
          ) : (
            <>
              <div className="chat-conversation-header">
                <div className="chat-contact-avatar" style={{ background: `${currentStage?.color || '#5CB8B2'}25`, overflow: 'hidden' }}>
                  {lead.profile_pic_url ? (
                    ...
                    <User size={16} style={{ color: currentStage?.color || '#5CB8B2' }} />
                  )}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>{lead.name || 'Sem nome'}</div>
                  {lead.phone && <div style={{ fontSize: 11, color: '#718096', display: 'flex', alignItems: 'center', gap: 4 }}><Phone size={10} />{lead.phone}</div>}
                </div>
              ...
              <div className="chat-messages">
                {messages.length === 0 && <div style={{ textAlign: 'center', color: '#A0AEC0', padding: 40, fontSize: 13 }}>Nenhuma mensagem</div>}
                {messages.map(m => (
                  <div key={m.id}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
              {rightTab === 'info' && (
                <>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 10, color: '#718096', textTransform: 'uppercase', marginBottom: 4 }}>Etapa</div>
                  ...
                  {user?.role !== 'atendente' && (
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 10, color: '#718096', textTransform: 'uppercase', marginBottom: 4 }}>Atendente</div>
                  ...
                  <div className="card" style={{ padding: 12, marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <div style={{ fontSize: 10, color: '#718096', textTransform: 'uppercase' }}>Informacoes</div>
                      {!editing ? (
                    ...
                    {editing ? (
                      ...
                      <div style={{ fontSize: 11, display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <div><span style={{ color: '#A0AEC0' }}>Nome:</span> {lead.name || '-'}</div>
                        <div><span style={{ color: '#A0AEC0' }}>Tel:</span> {lead.phone || '-'}</div>
                        <div><span style={{ color: '#A0AEC0' }}>Email:</span> {lead.email || '-'}</div>
                        <div><span style={{ color: '#A0AEC0' }}>Cidade:</span> {lead.city || '-'}</div>
                        <div><span style={{ color: '#A0AEC0' }}>Fonte:</span> {lead.source || '-'}</div>
                        <div><span style={{ color: '#A0AEC0' }}>Criado:</span> {new Date(lead.created_at).toLocaleDateString('pt-BR')}</div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="card" style={{ padding: 12, marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                      <div style={{ fontSize: 10, color: '#718096', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 3 }}><TagIcon size={10} /> Tags</div>
                        ...
                        {showTagMenu && (
                          <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, background: 'var(--bg-card)', border: '1px solid var(--border-medium)', borderRadius: 8, padding: 6, zIndex: 50, minWidth: 200 }}>
                            {availableTags.length > 0 && (
                              <>
                                {availableTags.map(t => (
                                  <button key={t.id} onClick={() => handleAddTag(t.id)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', border: 'none', background: 'none', color: '#2D3748', fontSize: 11, cursor: 'pointer', width: '100%', textAlign: 'left' }}>
                    ...
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                      ...
                      {(!lead.tags || lead.tags.length === 0) && <span style={{ fontSize: 10, color: '#A0AEC0' }}>Sem tags</span>}
                    </div>
                  </div>
                  ...
                  <div className="card" style={{ padding: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                      <div style={{ fontSize: 10, color: '#718096', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 3 }}><ListOrdered size={10} /> Cadencia</div>
                      <div style={{ position: 'relative' }}>
                        ...
                        {showCadenceMenu && (
                          <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, background: 'var(--bg-card)', border: '1px solid var(--border-medium)', borderRadius: 8, padding: 4, zIndex: 50, minWidth: 180, maxHeight: 220, overflowY: 'auto' }}>
                            {cadences.length === 0 && <div style={{ padding: 8, fontSize: 11, color: '#718096' }}>Nenhuma cadencia. Crie em /cadences</div>}
                            {cadences.map(c => (
                              <button key={c.id} onClick={() => handleAssignCadence(c.id)} style={{ display: 'block', padding: '6px 10px', border: 'none', background: 'none', color: '#2D3748', fontSize: 11, cursor: 'pointer', borderRadius: 4, width: '100%', textAlign: 'left' }}>
                                {c.name} <span style={{ color: '#A0AEC0' }}>({c.attempts.length} etapas)</span>
                              </button>
                            ))}
                          </div>
                        )}
                    ...
                    {leadCadence ? (
                      <>
                        ...
                        {leadCadence.status === 'completed' ? (
                          ...
                          <>
                            <div style={{ fontSize: 11, color: '#5CB8B2', marginTop: 2 }}>Etapa {(leadCadence.attempt_position ?? 0) + 1}/{leadCadence.total_attempts}: {leadCadence.action_type?.toUpperCase()}</div>
                            {leadCadence.attempt_description && <div style={{ fontSize: 11, color: '#2D3748', marginTop: 2, fontWeight: 500 }}>{leadCadence.attempt_description}</div>}
                            {leadCadence.attempt_instructions && <div style={{ fontSize: 10, color: '#718096', marginTop: 2, fontStyle: 'italic' }}>{leadCadence.attempt_instructions}</div>}
                            {leadCadence.attempt_message ? (
                              <>
                                <div style={{ marginTop: 8, padding: 8, background: 'rgba(92,184,178,0.05)', border: '1px solid rgba(92,184,178,0.2)', borderRadius: 6, fontSize: 11, color: '#4A5568', whiteSpace: 'pre-wrap', maxHeight: 140, overflowY: 'auto' }}>
                  ...
                  {/* Archive */}
                  <div className="card" style={{ padding: 12, marginTop: 12 }}>
                    <div style={{ fontSize: 10, color: '#718096', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 3, marginBottom: 6 }}><Archive size={10} /> Arquivar</div>
                    <div style={{ fontSize: 11, color: '#A0AEC0', marginBottom: 8 }}>Some do pipeline e do chat. Historico preservado. Ideal para contatos pessoais.</div>
              ...
              {rightTab === 'notes' && (
                <>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {notes.map(n => (
                      <div key={n.id} style={{ padding: '8px 10px', background: 'rgba(92,184,178,0.05)', borderRadius: 6, border: '1px solid rgba(92,184,178,0.1)' }}>
                        <div style={{ fontSize: 12 }}>{n.content}</div>
                        <div style={{ fontSize: 9, color: '#A0AEC0', marginTop: 2 }}>{n.user_name} · {new Date(n.created_at).toLocaleString('pt-BR')}</div>
                      </div>
                    ))}
                    {notes.length === 0 && <div style={{ textAlign: 'center', color: '#A0AEC0', padding: 20, fontSize: 11 }}>Sem notas</div>}
                  </div>
                </>
              )}
                ...
                <>
                  {history.map((h, i) => (
                    <div key={h.id} style={{ display: 'flex', gap: 8, padding: '6px 0', borderBottom: i < history.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
                      <Clock size={10} style={{ color: '#5CB8B2', marginTop: 3, flexShrink: 0 }} />
                      <div>
                        <div style={{ fontSize: 11 }}>{h.from_stage_name ? `${h.from_stage_name} → ${h.to_stage_name}` : `Entrada: ${h.to_stage_name}`}</div>
                        <div style={{ fontSize: 9, color: '#A0AEC0' }}>{h.trigger_type}{h.user_name ? ` · ${h.user_name}` : ''}</div>
                        <div style={{ fontSize: 9, color: '#A0AEC0' }}>{new Date(h.created_at).toLocaleString('pt-BR')}</div>
                      </div>
                    </div>
                  ))}
                  {history.length === 0 && <div style={{ textAlign: 'center', color: '#A0AEC0', padding: 20, fontSize: 11 }}>Sem historico</div>}
                </>
              )}
            </div>
          </div>
