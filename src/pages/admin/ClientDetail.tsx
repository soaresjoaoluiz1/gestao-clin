    ...
    <div>
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div><h1><Building2 size={20} style={{ marginRight: 8 }} />{account.name}</h1><div style={{ fontSize: 12, color: '#718096' }}>Slug: {account.slug}</div></div>
        </div>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id}>
                ...
                <td>{u.email}</td>
                <td><span className="stage-badge" style={{ background: u.role === 'gerente' ? '#5CB8B220' : '#5DADE220', color: u.role === 'gerente' ? '#5CB8B2' : '#5DADE2' }}>{u.role}</span></td>
            ...
            {users.length === 0 && <tr><td colSpan={5} style={{ textAlign: 'center', padding: 30, color: '#A0AEC0' }}>Nenhum usuario</td></tr>}
          </tbody>
        </table></div>
      </section>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {funnels.map(f => (
            <div key={f.id} className="card">
              <div style={{ fontWeight: 600, marginBottom: 8 }}>{f.name} {f.is_default ? <span style={{ fontSize: 10, color: '#5CB8B2' }}>PADRAO</span> : null}</div>
