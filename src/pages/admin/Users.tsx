          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id}>
                ...
                <td><span className="stage-badge" style={{ background: u.role === 'super_admin' ? '#FF6B6B20' : u.role === 'gerente' ? '#5CB8B220' : '#5DADE220', color: u.role === 'super_admin' ? '#FF6B6B' : u.role === 'gerente' ? '#5CB8B2' : '#5DADE2' }}>{u.role}</span></td>
