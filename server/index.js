...
app.get('/api/events', async (req, res) => {
  ...
  try {
    ...
    user = jwtMod.default.verify(token, process.env.JWT_SECRET || 'gestao-clin-secret-2026')
...
if (fs.existsSync(distPath)) {
  app.use('/gestao-clin', (req, res, next) => {
  ...
  // SPA fallback: any /gestao-clin/* route that doesn't match API → serve index.html
  app.get('/gestao-clin/*', (req, res) => {
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate')
    res.sendFile(resolve(distPath, 'index.html'))
  })
  console.log('[Gestão Clin] Serving frontend from /gestao-clin/')
}

app.listen(PORT, () => {
  console.log(`[Gestão Clin API] Running on http://localhost:${PORT}`)
  startScheduler()
})
